...
export interface UseRequestReturn<Q, T, C> {
   ...
   */
  request: (
    queryOrParams?: MaybeRefOrGetter<Q>,
    config?: MaybeRefOrGetter<C>,
  ) => Promise<void>
}
...
>(
  ...
  config?: MaybeRefOrGetter<C & { immediate: boolean }>,
  ...
  const request = async (
    queryOrParams: MaybeRefOrGetter<Q> = _queryOrParams!,
    config: MaybeRefOrGetter<C> = _config! as MaybeRefOrGetter<C>,
  ) => {
    const _queryOrParams = resolveUnref(queryOrParams)
